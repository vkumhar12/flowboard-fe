...
// "/" stays inside this app (the homepage); any other "/"-prefixed path
// belongs to the dashboard app and needs a full cross-origin URL.
const isDashboardRoute = (href: string): boolean => href.startsWith("/") && href !== "/";
          ...
          <div key={col.heading}>
            ...
            <ul className="mt-4 space-y-2.5">
              {col.links.map(([label, href]) => (
                <li key={label}>
                  {isDashboardRoute(href) ? (
                    <a href={dashboardHref(href)} className="text-xs font-medium text-muted transition-colors hover:text-ink">
                      {label}
                    </a>
                  ) : (
